...
from corecoder.agent.dag.models import TaskNode, TaskStatus, ExecutionResult
from corecoder.agent.dag.graph import TaskGraph
from corecoder.agent.dag.recovery import RecoveryManager, RecoveryAction
...
from corecoder.agent.workflow.executor import Executor, TaskContext
from corecoder.agent.dag.memory import MemoryInjector, WorkingMemory
...
class Scheduler:
    ...
    async def _execute_single(self, task: TaskNode) -> SchedulingDecision:
        ...
        # Inject downstream task titles so the executor's task contract
        # can show the agent what NOT to do.
        downstream_ids = self.graph.get_dependents(task.id)
        downstream_titles = []
        for did in downstream_ids:
            dn = self.graph.get_node(did)
            if dn:
                downstream_titles.append(dn.title)
        if downstream_titles:
            memory.downstream_tasks = downstream_titles
        ...
        verification = self.verifier.verify(
            ...
            patch=patch,
            working_dir=".",
            task_meta={"title": task.title, "description": task.description},
        )
