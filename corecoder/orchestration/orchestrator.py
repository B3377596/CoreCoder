...
class Orchestrator:
    ...
    def _build_verifier(self) -> BaseVerifier:
        ...
        Always includes cheap (no-subprocess) verifiers:
        - OutputVerifier: checks LLM output for required/forbidden patterns
        - FileExistsVerifier: checks that expected files were created

        Optionally includes subprocess verifiers:
        - TestVerifier: if test_command is configured
        - LintVerifier: if lint_command is configured

        Verification instructions are generated by the LLMPlanner in each
        task's metadata (e.g., "expected_files", "required_patterns").
        The verifiers read these automatically.
        """
        from corecoder.orchestration.engine.verifier import (
            OutputVerifier,
            FileExistsVerifier,
        ...
        # Layer 1: Always check output patterns and file existence.
        # These are zero-cost — no subprocesses, just string matching and
        # filesystem stat calls.  The LLMPlanner writes verification hints
        # into task metadata, and these verifiers consume them.
        composite.add(OutputVerifier())
        composite.add(FileExistsVerifier())

        # Layer 2: Subprocess verifiers — run test/lint commands when configured
